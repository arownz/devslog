...
export default function PostCard({ id, image, created_at, author, title, upvotes, downvotes, comments, isLoggedIn, onBookmark, isBookmarked, layout, onClick, onVote }) {
  ...
  const handleVote = async (e, voteType) => {
    e.stopPropagation(); // Prevent the click from bubbling up to the article
    if (!isLoggedIn) {
    ...
    try {
      ...
      if (data.success) {
        if (voteType === 'upvote') {
          ...
          if (localDownvotes > 0) setLocalDownvotes(prev => prev - 1);
        } else {
          setLocalDownvotes(prev => prev + 1);
          if (localUpvotes > 0) setLocalUpvotes(prev => prev - 1);
        }
      ...
      <article className={cardClass} onClick={() => {
        ...
        <div className={contentClass}>
          ...
          <div className="flex justify-between items-center">
            ...
            <button onClick={onBookmark} className={`text-gray-600 hover:text-yellow-500 ${isBookmarked ? 'text-yellow-500' : ''}`}>
